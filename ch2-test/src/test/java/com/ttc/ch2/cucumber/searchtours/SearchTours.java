...
public class SearchTours {
	...
	public void execute_search_tours() throws IOException {
	    ...
	    request.setNumberOfRecords(new BigInteger("100"));
		...
		switch(testParameter) {
			case "Continents":
				...
				for(Map<String,Object> row: result) {
					...
					for(SearchResults sr: response.getSearchResults()) {
						if(sr.getPriceFrom() < prevPrice) {
							isResultsValid=false;
							break;
						}
						prevPrice = sr.getPriceFrom(); 
