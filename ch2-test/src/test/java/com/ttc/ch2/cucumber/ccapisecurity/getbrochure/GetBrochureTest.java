...
public class GetBrochureTest {
	...
	private GetBrochureRequest request = new GetBrochureRequest();
	private GetBrochureResponse response;	
	...
	public void setSellingComapnyCode(String comapnyCode) throws Throwable {
		request.setSellingCompanyCode(comapnyCode);	
	}
	...
	public void before()
	{		
		request.setAgentImage("n/a");
		request.setAgentText("n/a");
		request.setBrandCode("CH");	
		...
		try
		{
			...
			{
				Function f =(Function) s.createQuery("from Function where name='"+FunctionType.GET_BROCHURE.getSimpleName()+"'").uniqueResult();
				SellingCompany sc =(SellingCompany) s.createQuery("from SellingCompany where code='CHCANS'").uniqueResult();
