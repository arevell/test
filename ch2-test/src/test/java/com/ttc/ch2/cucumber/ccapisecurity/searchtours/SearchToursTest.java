...
public class SearchToursTest {
	...
	private SearchToursRequest request = new SearchToursRequest();
	private SearchToursResponse response;	
	...
	public void mainTest() {
			response = wsClient.port.searchTours(request);
	}
	...
	{		
		...
		request.setOrderBy("n/a");
		request.setOrderDirection("n/a");	
		request.setPreferedRoomType("n/a");	
		request.setPriceFrom(new Double(0));	
		request.setPriceTo(new Double(0));
		request.setContinentCodes("AFRI");
		request.setCountryCodes("PL");
		request.getKeywordsAndPhrases().add("n/a");
