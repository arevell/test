<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="intro2">
		<fo:page-sequence master-reference="standard">
			...
			<fo:flow flow-name="xsl-region-body">
				...
				<fo:block xsl:use-attribute-sets="font-serif" color="red"
					font-size="24pt" text-align="left" margin-top="0mm" padding-top="5mm"
					margin-left="5mm">
				...
				<xsl:call-template name="block2">
					...
					<xsl:with-param name="top" select="'132mm'" />
				</xsl:call-template>
				...
				<xsl:call-template name="block2">
					...
					<xsl:with-param name="left" select="'7.5mm'" />
					<xsl:with-param name="top" select="'158mm'" />
				</xsl:call-template>
				...
				<xsl:call-template name="block2">
					...
					<xsl:with-param name="left" select="'7.5mm'" />
					<xsl:with-param name="top" select="'184mm'" />
				</xsl:call-template>
				...
				<xsl:call-template name="block2">
					...
					<xsl:with-param name="left" select="'7.5mm'" />
					<xsl:with-param name="top" select="'210mm'" />
				</xsl:call-template>
	...
	<xsl:template name="block2">
		...
		<fo:block-container position="absolute" height="23mm"
				...
				<fo:list-block provisional-label-separation="2mm"
					...
					<fo:list-item>
						...
						<fo:list-item-body start-indent="body-start()">
							<fo:block font-size="10pt" color="red" font-weight="bold">
