<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<fo:block margin-top="1mm" font-size='9pt'
										margin-left="4mm" font-family="AvenirLTStd-Light">
										<xsl:text>Please enquire for
	...
	<xsl:template name="departurestable">
		<fo:block-container>
			<fo:table table-layout="fixed" width="110mm" margin-top="1mm"
				margin-left="1mm">
				...
				<fo:table-header font-family="AvenirLTStd-Heavy"  font-size="9pt"
					font-weight="bold">
				...
				<fo:table-body font-family="AvenirLTStd-Light" font-size="9pt">
					...
					<!-- this is a collection of all the departures, so we can refer to 
					...
					<xsl:for-each select="$dates">
							...
							<xsl:for-each select="$rates1">
								<xsl:sort
									...
									data-type="number" />
								<xsl:if test="di:DeparturePricings/di:DeparturePricing[@SellingCompanyCode=$company] and (position() = 1)">
									<fo:table-row>
	...
	<xsl:template name="rate-cells">
		...
		<xsl:variable name="prices"
			select="$rate/di:DeparturePricings/di:DeparturePricing[@SellingCompanyCode=$brochure/@SellingCompany]" />
