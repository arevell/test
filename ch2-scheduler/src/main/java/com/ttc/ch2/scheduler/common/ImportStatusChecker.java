...
public class ImportStatusChecker implements JobStatusChecker{

	private static final Logger logger = LoggerFactory.getLogger(ImportStatusChecker.class);
...
//		this.sessionFactory=ctx.getBean("dataSource", SessionFactory.class);
	...
	public boolean isCancelProcess() {
		...
		Preconditions.checkState(StringUtils.isNotEmpty(brandCode) , "ImportStatusChecker->isCancelProcess field brandCode is not initialize");	
