 ...
 * */
@Component("TourDepartureRepositoryOld")
@Scope("request")
public class TourDepartureRepositoryOld  extends BaseTourRepository
{	
	private static final Logger logger=LoggerFactory.getLogger(TourDepartureRepositoryOld.class);
    private static final Logger activityLogger = LoggerFactory.getLogger("ch2.activity.TourDepartureRepositoryOld");
	
	@Init
	public void init() throws Exception {	 		
		logger.trace("TourDepartureRepositoryOld:init-start");
		initializeForm();
		initializeListModel();
		logger.trace("TourDepartureRepositoryOld:init-end");
	}
	
	@Command("onPagingList")
	@NotifyChange("listModel")
	public void onPagingList(BindContext ctx) {	
		logger.trace("TourDepartureRepositoryOld:onPagingList-start");
		...
		logger.trace("TourDepartureRepositoryOld:onPagingList-end");
	}
	
	@Command("download")
	public void download(@BindingParam("element") ContentRepository selected) {
		logger.trace("TourDepartureRepositoryOld:download-start");
		...
		logger.trace("TourDepartureRepositoryOld:download-end");
	}
	
	@Command
	@NotifyChange("listModel")
	public void onSearch(@BindingParam("element") SimpleForm form) {
		logger.trace("TourDepartureRepositoryOld:onSearch-start");
		searchTable(form);
		logger.trace("TourDepartureRepositoryOld:onSearch-end");
	}
