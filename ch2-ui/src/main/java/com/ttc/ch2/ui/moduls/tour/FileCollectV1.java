...
import com.ttc.ch2.ui.moduls.tour.services.FileCollectV1WeaveService;

@org.springframework.stereotype.Component("fileCollectV1")
@Scope("request")
public class FileCollectV1 extends BaseFileCollect {

	private static final Logger logger = LoggerFactory.getLogger(FileCollectV1.class);
	private static final Logger activityLogger = LoggerFactory.getLogger("ch2.activity.FileCollectV1");

	@Inject
	protected FileCollectService fileCollectService;

	@Inject 
	protected FileCollectV1WeaveService service;
	...
	public void init() {
		...
		try {
			listFileCollectVOs = fileCollectService.getList(FileCollectVersion.V1);
	...
	public String generateLink(SellingCompany selected){		
		return "/"+PathType.O_ARCHIVES.getPartPath()+"/"+selected.getCode()+".zip?token="+SecurityHelper.getToken();
	}
