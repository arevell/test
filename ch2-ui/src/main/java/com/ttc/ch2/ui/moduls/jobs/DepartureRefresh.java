...
@Component("DepartureRefresh")
@Scope("request")
public class DepartureRefresh {
	
	private static final Logger logger=LoggerFactory.getLogger(DepartureRefresh.class);
	...
	public void init() throws SchedulerServiceException {				
		...
		try {			
			...
			this.form=new SchedulerForm(brands);
			...
			if(filter==null){
				filter=new QuartzJobHistory();
				SessionHelper.putAttributeToUserContext(UserContextStaticName.FORM_LIST_JOB, filter);
			}			
			if(condition==null){
					condition=new QueryCondition(0, PagingQuartzHistoryListModel.PAGE_SIZE,new SortCondition("startDate", Direction.DESC));
...
//			schedulerService.changeJobTime(form.getChgTime(),form.getBrandCode());
	...
	{		
	 ...
	 public void changeFilter() {
		 	...
		 	filter.setQuartzJob(new QuartzJob());
		 	...
		 	filter.setExecutedBy(quartzJobHistoryForm.getExecutedBy());
		 	filter.getQuartzJob().setUiName(quartzJobHistoryForm.getUiName());
		 	filter.getQuartzJob().setGroupName(quartzJobHistoryForm.getJobGrName());
		 ...
		 try{
			 if(schedulerService.resetAllowed(brandCode)==false)
			 {
				...
				disableAllButtons=true;
				 Messagebox.show(Labels.getLabel("jobs.reset.msg.description") , Labels.getLabel("jobs.reset.msg.title"),Messagebox.OK | Messagebox.CANCEL,Messagebox.EXCLAMATION, new EventListener<Event>() {			
						@Override
