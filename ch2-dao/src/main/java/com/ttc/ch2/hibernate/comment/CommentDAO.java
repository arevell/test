...
public class CommentDAO<T, ID extends Serializable> extends  GenericDAOImpl<T, ID> {
	...
	{
		if(filter != null) {			
			if(filter.getModifiedTime() != null) {			
				search.addFilterGreaterOrEqual("modifiedTime", filter.getModifiedTime());				
			}
			
			if(StringUtils.isNotBlank(filter.getMessage())) {
				search.addFilterILike("message", "%" + filter.getMessage()+"%");
			}
			
			if(StringUtils.isNotBlank(filter.getMessageCode())) {
				search.addFilterILike("messageCode", "%" + filter.getMessageCode()+"%");
			}			
		}
	}
