...
import com.ttc.ch2.bl.departure.TourDepartureService;
import com.ttc.ch2.bl.departure.TourDepartureServiceException;
...
public class ReconciliationReportServiceImpl implements ReconciliationReportService {
	...
	private static Logger logger = LoggerFactory.getLogger(ReconciliationReportServiceImpl.class); 
...
//		List<ContentRepository> contentRepositoryList = contentRepositoryService.getContentRepositoriesList(new QueryCondition(), filter, RepositoryStatus.TourInfoOnly, RepositoryStatus.TourDepartureOnly, RepositoryStatus.TIandTD);
...
//				contentRepository.getXmlContentRepository().clear();
		...
		} catch ( TourDepartureServiceException | XMLStreamException | FactoryConfigurationError | JAXBException e) {
	...
	private String preparteHtmlBody(String brandCode, IndexSynchronizerVO indexSynchronizerVO, FileCollectVO fileCollectVO, List<String> toursListTourInfo, List<String> toursListTourDeparture, List<String> toursListBoth, Map<String, List<String>> sellingCompanyToursMap) {
		...
		if (indexSynchronizerVO == null) {
			stringBuilder.append(String.format(HTML_REPORT_MAIL_TABLE, "error", "Elastic Search indexing", "error", String.format(HTML_REPORT_MAIL_ROW, "None of the tours have been indexed")));
		} else if (!indexSynchronizerVO.hasErrors()) {
			stringBuilder.append(String.format(HTML_REPORT_MAIL_TABLE, "success", "Elastic Search indexing", "success", String.format(HTML_REPORT_MAIL_ROW, "All tours have been successfully indexed")));
		} else {
